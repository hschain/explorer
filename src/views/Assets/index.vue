    ...
    <el-card shadow="never" class="table">
      <div class="searchBar">
        <div class="queryIndex">
          <el-input class="inputKeyword" v-model="textarea" :placeholder="$t('assets.searchInputPlaceholder')"
            clearable></el-input>
        </div>
      </div>
      <!-- 小分页组件 -->
      <small-pagination :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.size" @pagination="pagination"></small-pagination>
      <el-table v-loading="loading" :default-sort="{prop: 'value', order: 'descending'}" :data="AssetsList" stripe
        style="width: 100%">
        ...
        <!-- 流通量 -->
        <el-table-column sortable prop="minted_supply" :label="$t('assets.circulationNum')">
          <template slot-scope="scope">
            <span>{{scope.row.minted_supply}}</span>
          </template>
        </el-table-column>
      ...
      <Pagination :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.size" @pagination="pagination" />
    </el-card>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        this.$http(this.$api.getCurrency).then((res) => {
          if (res.code === 200) {
            ...
            this.oAssetsList.forEach(item => {
              ...
              item.value = (item.price * item.minted_supply).toFixed(6)
            })
            if (this.textFilter) {
              this.AssetsList = this.oAssetsList.filter((item) =>
      ...
      handlePage() {
        if (this.AssetsList.length > this.listQuery.size) this.AssetsList = this.oAssetsList.slice((this.listQuery.page -
          1) * this.listQuery.size, this.listQuery.page * this.listQuery.size)
      },
      pagination(val) {
        this.listQuery = {
          page: val.page,
